...
RUN python -m pip install --no-cache-dir . \
    && mkdir -p /app/runtime/outputs /app/runtime/kex-proofs \
...
HEALTHCHECK --interval=30s --timeout=8s --start-period=20s --retries=3 \
  CMD python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8091/health', timeout=5)" || exit 1

CMD ["gunicorn", "--workers", "2", "--threads", "4", "--timeout", "120", "--bind", "0.0.0.0:8091", "--access-logfile", "-", "--error-logfile", "-", "klean_pod_checker.wsgi:app"]
